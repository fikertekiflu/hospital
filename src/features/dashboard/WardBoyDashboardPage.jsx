...
import {
  ...
  MagnifyingGlassIcon,
  PencilSquareIcon, // For notes or other actions
  WrenchScrewdriverIcon // For reporting issues
...
const WardBoyDashboardPage = () => {
  const { currentUser, token } = useAuth();
  const navigate = useNavigate();
  const queryClient = useQueryClient();

  const wardBoyId = currentUser?.linkedStaffId; // Assuming linkedStaffId in SystemUser is the ward_boy_id

  // Fetch active tasks for the logged-in Ward Boy
  const fetchMyActiveAssignments = async () => {
    if (!token || !wardBoyId) return [];
  ...
  const {
    ...
    queryKey: ['myActiveAssignmentsWardBoy', wardBoyId], // Unique queryKey for WardBoy
    queryFn: fetchMyActiveAssignments,
    enabled: !!token && !!wardBoyId && currentUser?.role === 'WardBoy',
  ...
  const updateAssignmentStatusMutation = useMutation({
    ...
    onSuccess: (data, variables) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['myActiveAssignmentsWardBoy', wardBoyId] });
    },
  ...
  if (!currentUser?.isLoading && !wardBoyId && currentUser?.role === 'WardBoy') {
    return (
        <div className="p-6 sm:p-8 bg-white rounded-lg shadow-xl max-w-lg mx-auto mt-10 text-center">
            ...
            <p className="mt-2 text-slate-600">
                Your system user account is not yet linked to a Ward Boy's professional profile.
  ...
  const tasksToday = activeAssignments?.filter(task => task.assignment_start_datetime && isToday(parseISO(task.assignment_start_datetime))) || [];
  const upcomingTasks = activeAssignments?.filter(task => task.assignment_start_datetime && !isToday(parseISO(task.assignment_start_datetime))) || [];


  return (
    <div className="space-y-8 p-1 animate-fadeIn">
      <header className="mb-8">
        <h1 className="text-4xl font-bold text-slate-800 tracking-tight">Ward Boy Dashboard</h1>
        <p className="mt-2 text-lg text-slate-600">
          Welcome, {currentUser?.full_name || currentUser?.username}! Here are your assigned duties.
        </p>
      </header>
      ...
      <section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <ActionCard title="View Full Task List" description="See all your pending and in-progress tasks." href="/assignments/my-tasks" icon={ClipboardDocumentListIcon} bgColor="bg-orange-600" hoverBgColor="hover:bg-orange-700" iconColor="text-orange-100" />
          <ActionCard title="Patient Lookup" description="Find patient records for location or assistance." href="/patients" icon={MagnifyingGlassIcon} bgColor="bg-sky-600" hoverBgColor="hover:bg-sky-700" iconColor="text-sky-100" />
          <ActionCard title="Report Issue" description="Log equipment or ward issues." href="/wardboy/reports/new" icon={WrenchScrewdriverIcon} bgColor="bg-gray-600" hoverBgColor="hover:bg-gray-700" iconColor="text-gray-100" />
        </div>
      </section>
      ...
      <section className="bg-white p-4 sm:p-6 rounded-xl shadow-xl">
        <div className="flex justify-between items-center mb-5">
            ...
            <h2 className="text-xl font-semibold text-slate-700">Today's Active Tasks ({format(new Date(), 'PPP')})</h2>
            <Link
                ...
                <div className="mt-3 pt-3 border-t border-slate-200 flex flex-wrap gap-2 items-center justify-end">
                    ...
                    {task.status === 'In Progress' && (
                     ...
                     <button
                        ...
                        className="text-xs inline-flex items-center px-2.5 py-1 border border-transparent rounded-md shadow-sm text-gray-700 bg-gray-100 hover:bg-gray-200 transition-colors"
                        title="Add/View Task Notes"
                    >
...
export default WardBoyDashboardPage;
